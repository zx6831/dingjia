...
class VectorManager:
    ...
    def _hard_delete(self) -> None:
        ...
        if self.db:
            self.db.save_local(self.store_dir)
            self._rebuild_id_map_from_db(assign_seq=True)
            logger.info(f"???????? {len(removed_ids)} ??id_map ???")
    ...
    def show_index(self, limit: int = 20, preview_chars: int = 120, show_metadata: bool = False) -> List[Dict[str, Any]]:
        """
